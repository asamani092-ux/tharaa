...
import { LogOut, BookOpen, Users, BarChart3, Settings, Layers, Home } from "lucide-react";
...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground flex-col md:flex-row" dir="rtl">
      {/* Sidebar */}
      <aside className="w-full md:w-64 flex flex-col shrink-0" style={{ backgroundColor: 'hsl(218,42%,10%)', borderLeft: '1px solid hsl(217,36%,18%)' }}>
        <div className="p-5 flex justify-center" style={{ borderBottom: '1px solid hsl(217,36%,18%)' }}>
          <img src={logoPath} alt="ثراء المعرفة" className="h-14 object-contain" />
        </div>
        ...
        <div className="p-4" style={{ borderTop: '1px solid hsl(217,36%,18%)' }}>
          <div className="mb-3 px-2 text-sm text-muted-foreground font-medium" data-testid="text-username">
            {session?.user?.name}
          </div>
          <Button
...
export function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col" dir="rtl">
      <header style={{ backgroundColor: 'hsl(218,42%,10%)', borderBottom: '1px solid hsl(217,36%,18%)' }}>
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img src={logoPath} alt="ثراء المعرفة" className="h-10 object-contain" />
            <div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-foreground/80 hidden sm:block" data-testid="text-username">
              {session?.user?.name}
            </span>
            <Button
              data-testid="button-logout"
